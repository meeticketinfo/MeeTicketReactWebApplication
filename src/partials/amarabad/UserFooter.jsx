...
function UserFooter() {
  return (
    <footer className="bg-gradient-to-b from-[#394D48] to-[#262F20] border-t border-[#D0D7CE] pt-8 text-[#FDFAF7]">
      <div className="container mx-auto px-3">
        <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-8 pb-12">
          ...
          <div className="flex flex-col md:flex-row md:items-start md:w-2/5 gap-3">
            <img src={MeeTicketLogo} alt="Archaeological Survey of India Logo" width={85} className="rounded-lg" />
            <div>
              <div className="text-lg lg:text-xl xl:text-2xl font-normal text-[#FDFAF7] mb-2 mt-3">Quick Tickets & Easy Entry - Just Tap, Scan, and Go!</div>

              <div className="flex gap-6 mt-4 md:mt-10">
                {/* Facebook */}
                <a href="https://www.facebook.com/amrabadtigerreserve/" target='_blank' rel="noreferrer" aria-label="Facebook" className="text-[#D0D7CE] hover:text-[#FDFAF7]">
                ...
                {/* Twitter */}
                <a href="https://x.com/AmrabadTiger" target='_blank' rel="noreferrer" aria-label="X (Twitter)" className="text-[#D0D7CE] hover:text-[#FDFAF7]">
          ...
          <div className="flex flex-wrap md:flex-nowrap flex-1 justify-between gap-8">
            {footerLinks.map((col) => (
              <div key={col.title}>
                <div className="font-extrabold text-base mb-4 text-[#FDFAF7]">{col.title}</div>
                <ul className="space-y-2 text-[#D0D7CE]">
                  {col.links.map((link) => (
                    <li key={link.label}>
                      <Link to={link.href} target={link.target} className="hover:text-[#FDFAF7] hover:underline">{link.label}</Link>
                    </li>
                  ))}
                </ul>
              </div>
      ...
      {/* Copyright bar */}
      <div className="bg-[#262F20] py-6 text-center text-sm text-[#D0D7CE]">
        <div className="container mx-auto">
          <span>Copyright © 2025 <span className="font-bold text-[#FDFAF7]">MEETICKET</span> | Content owned, maintained and updated by <span className="font-bold text-[#FDFAF7]">MEETICKET</span></span>
        </div>
      </div>
    </footer>
  );
