...
const MetroUserReport = () => {
    ...
    },
    {
      ...
      cellRenderer: (params) => (
        <Link
          ...
          to={`/metro-user-detailed-report?mobileNumber=${
  ...
  const loadUserReport = (page = 0) => {
    fetchMetroUserReport({
      ...
      PageIndex: page + 1, // convert zero-indexed to 1-indexed
      pageSize: PAGE_LIMIT,
    });
  };
  ...
  return (
    <AdminLayout>
      <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <MetroUserReportForm PageIndex={1} pageSize={PAGE_LIMIT} SetcurrentPage={setCurrentPage} />
        <div>
          <AgGridTable
            ExportName="UserStatusTransactionReport"
            rowData={metroUserReport?.data}
            columnDefs={columnDefs}
            isFetchLoading={isfetchMetroUserReport}
            isPagination={false}
            tableHeight={metroUserReport?.data?.length > 10 ? 560 : 330}
            IsReactPaginate={true}
            setPageLimit={setPAGE_LIMIT}
            pageLimit={PAGE_LIMIT}
            handlePageClick={handlePageClick}
            currentPage={currentPage}
            totalCount={metroUserReport?.totalCount}
            showTotalCount={true}
...
export default MetroUserReport;
