...
const FailedTransactionsForm = () => {
  const [searchParams, setSearchParams] = useSearchParams();

  const { allEntityTypes, fetchAllEntityTypes } = useEntityTypesStore();
  const { allDepartmentTypes, fetchAllDepartmentTypes } = useDepartmentTypesStore();
  const { allParks, fetchAllParks } = useParkStore();
  const {isFetchFailureUserTransactionReport, fetchFailureUserTransactionReport} = userFailureTransaction();
  ...
  const onSubmit = (values) => {
    ...
    fetchFailureUserTransactionReport({
      ...
      category: values.status == "" ? "" : values.status == "CONFIRMED" ? "" : searchParams.get("category") || ""
    });
  };
...
export default FailedTransactionsForm;