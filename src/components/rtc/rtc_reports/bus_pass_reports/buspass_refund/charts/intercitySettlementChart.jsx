...
const IntercitySettlementChart = ({
  data,
  title,
  angleKey,
  calloutLabelKey,
      ...
      <div className="flex justify-between">
        ...
        <div className="flex items-center gap-2 bg-[#C0DDFF] rounded-lg px-4 py-3 shadow-sm">
          <span className="text-lg text-[#404040] font-semibold">Total Settlement Summary</span>
            <Link to={`/intercity-settlement-summary-inner-report?${searchParams.toString()}&status=all`} className="font-semibold text-lg text-[#57a4d8] ml-2 underline">
            {totalCount}
          </Link>
        </div>
      </div>
                ...
                <tbody>
                  {data?.map((item, index) => (
                    <tr key={index} className="border-b border-gray-200 hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        <Link
                          to={`/intercity-settlement-summary-inner-report?${searchParams.toString()}&status=${encodeURIComponent(item.status)}`}
                          onClick={() => {
                            const newParams = new URLSearchParams(searchParams.toString());
                            newParams.set('status', item.status);
                            localStorage.setItem("intercitySettlementInnerTransactionSearchParams", newParams.toString())
                          }}
...
export default IntercitySettlementChart;
