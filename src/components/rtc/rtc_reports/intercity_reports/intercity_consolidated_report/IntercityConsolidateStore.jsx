...
export const useIntercityConsolidateStore = create((set) => ({
  ...
  fetchIntercityConsolidateData: async (payload) => {
    ...
    try {
      const params = `purchaseOrBooking=${payload.purchaseOrBooking}&fromDate=${payload.fromDate}&toDate=${payload.fromDate}&mobileNumber=${payload.mobileNumber}&typeOfBus=${encodeURIComponent(payload.typeOfBus)}&paymentMode=${payload.paymentMode}&transactionId=${payload.transactionId}&PNRNumber=${payload.PNRNumber}&departureLocation=${payload.departureLocation?payload.departureLocation:""}&arrivalLocation=${payload.arrivalLocation?payload.arrivalLocation:""}&pageNumber=${payload.pageNumber}&pageSize=${payload.PageSize}`;
      const method = "get";
